...
function updateComputerBoardCell(row, col, newState) {
    const table = document.getElementById(BOARD_IDS.computer);
    const cell = table.rows[row + HEADER_OFFSET].cells[col + HEADER_OFFSET];
...
async function placeShipAtCell(row, col) {
    ...
    if (data.isError) {
        document.getElementById(ELEMENT_IDS.computerMessage).innerText = "Invalid placement — try again.";
        return;
    }
...
window.onload = async function () {
    await fetch(API_PATHS.placementStart, { method: "POST" });
